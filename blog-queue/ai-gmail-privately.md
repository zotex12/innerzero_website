...
- [ ] Verify the canonical Gmail wording on /features still reads "Read-only Gmail with metadata only (sender, subject, snippet). Message bodies are never fetched or stored."
- [ ] Confirm Superhuman, Shortwave, and Gemini in Gmail still describe themselves as full-inbox cloud AI; update if positioning has shifted
- [ ] Re-check the Gmail OAuth scope reference (https://developers.google.com/gmail/api/auth/scopes) is still live
- [ ] Confirm InnerZero's connector still skips Spam, Trash, Drafts, Sent, Promotions, Social
- [ ] Refresh the Outlook / ProtonMail roadmap line if any of those have shipped
-->
---
