...
**[Claude Opus 4.7](/models)** released on 15 April 2026. At launch it was Anthropic's strongest reasoning model, and InnerZero's v0.1.4 release added it as a selectable model for anyone using **bring your own keys** with their own Anthropic account. (Since then Anthropic has shipped Opus 4.8, now the current top model, and you can select it the same way. See [/models](/models) for what's live today.) This guide walks through the setup, what it costs, and when you'd actually pick it over a model running on your own machine.
...
**Claude Opus 4.7 was Anthropic's most capable model in the Claude 4 family at release**, on 15 April 2026 (Opus 4.8 has since taken the top spot and selects the same way). It's the one you pick when reasoning quality matters more than speed or cost. Anthropic tuned it hard for long-context work, extended thinking, and coding. If you've tried Opus 4.6 and wanted the same behaviour but sharper, that's the upgrade.

InnerZero is a local-first AI assistant. The default setup runs entirely on your machine using [Ollama](https://ollama.com) or [LM Studio](/blog/innerzero-vs-lm-studio). Everything is free, stays on your hardware, and needs no internet connection once the model is downloaded. That's the normal mode.
...
| Reasoning quality | Top tier at launch | Good for most tasks |
...
InnerZero encrypts your [API key](/features/cloud-ai) and stores it on your machine using a key derived from your hardware ID. It never gets uploaded anywhere. If you uninstall InnerZero, the encrypted blob stays in your user data directory until you delete it yourself.
