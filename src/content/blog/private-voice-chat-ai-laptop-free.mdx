...
Two design tricks hide much of the extra latency. Transcription can start while you are still talking, and the voice can begin reading the first sentence before the model writes the rest, so a good [voice loop](/features/voice) feels quicker than the sum of its parts. The standalone panels for each step are covered in [local text to speech and speech to text](/blog/local-text-to-speech-speech-to-text).
