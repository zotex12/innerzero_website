...
InnerZero's default [voice mode](/features/voice) is fully local. There is also an opt-in Cloud Voice option that routes through OpenAI's voice API for users who specifically want a premium voice (currently 13 voices to pick from). It is genuinely opt-in. Default mode never touches a cloud voice service.
