...
description: "InnerZero is a free private AI assistant you install on your PC in minutes. One setup wizard, no account, and your chats never leave your machine."
date: "2026-05-07"
...
<PostImage
  ...
  alt="InnerZero setup complete screen showing the detected system tier, the chosen director model and voice model, and a measured speed of 18.2 tokens per second"
  width={1345}
  height={1122}
  caption="Setup done: the wizard reports your tier, models, and measured speed before first launch."
/>
...
The only paid tier for the desktop app is a per-seat Business Licence for commercial use: £19.99 per seat per month, or £149.99 per seat per year (dropping to £129.99 per seat per year at 5 or more seats). Education users, registered charities, and non-profits are exempt from that licence even when used at work. Optional cloud plans exist for users who want to call frontier models through InnerZero's managed proxy, but they are strictly optional and not required for any local feature.

The model files themselves are open-source releases from teams like Alibaba's [Qwen3 family on Hugging Face](https://huggingface.co/Qwen), Google's Gemma, and OpenAI's gpt-oss, distributed at no cost under their respective community licences. InnerZero downloads them through [Ollama](https://ollama.com) and stores them locally. Once the download finishes you own the bytes.
...
There is one optional cloud mode for users who want access to frontier models like the latest Claude and GPT. It is off by default and clearly labelled in the interface. When enabled, only the current prompt with relevant context is sent to the provider you chose. Your full memory database, your files, your past conversations, and your profile facts never leave the machine. There is also an in-app connections log that shows every outbound network call, so you can verify in real time what is and is not going out. The full breakdown lives in [how InnerZero stays private](/blog/how-innerzero-stays-private).
...
You also get document Q&A: drag a file into the chat and ask questions about it. Knowledge packs are an optional download for offline answers, with a curated Best of Wikipedia pack (about 871,000 articles) available so the assistant can answer factual questions with no internet connection at all. A full English Wikipedia pack is coming soon.
...
Personal use is free with no restrictions. For business use, the licence is per seat: £19.99 per month, or £149.99 per year (with a £129.99 per year volume rate from 5 seats). 25 or more seats are handled by sales contact. Registered charities, non-profits, and education users are exempt and can use it commercially without paying. There is no usage cap and no feature gate; the licence is just permission to use the local app commercially.
