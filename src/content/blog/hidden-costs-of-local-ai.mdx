...
That's 12 to 17 GB for a clean multi-model setup. Add a Wikipedia knowledge pack (Best of Wikipedia ~350 MB, or Simple English ~460 MB), a second chat model (another 5 GB), a specialist coding model at 14b (~9 GB), and you're at 30 GB easily. Not a dealbreaker in 2026 when 1 TB SSDs are standard. Worth noting if you're on a 256 GB MacBook that's already nearly full.
...
**Yes. New InnerZero releases re-download runtime files, and new model releases re-download weights.** A point release is a few hundred MB. A major release is the full 300 to 500 MB [installer](/features/setup). Model updates can be multiple GB.
