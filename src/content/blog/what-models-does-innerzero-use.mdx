...
- **Qwen3 1.7B** for CPU minimal tier (4 GB RAM, no graphics card)
- **Qwen3 4B** for basic tier (8 GB RAM, CPU-only) and entry tier (16 GB RAM with 8 GB VRAM)
- **Qwen3 8B** for CPU standard tier (16 GB RAM) and standard tier (32 GB RAM with 16 GB VRAM)
- **Qwen3 14B** for CPU high tier (32 GB RAM) and performance tier (32 GB RAM with 24 GB VRAM)
- **Qwen3 30B** for pro tier (64 GB RAM with 48 GB VRAM)

The "B" stands for billion parameters. Bigger models are smarter but need more [memory](/blog/ai-that-remembers). InnerZero's setup wizard handles the selection automatically. You don't need to know which one you're running.

Qwen3 is a strong all-rounder. It handles conversation, reasoning, tool use, coding help, and general knowledge well. It won't match the biggest frontier cloud models, but for a model running on consumer hardware, it's remarkably capable.
...
All the AI models run through [Ollama](https://ollama.com), an open-source tool that manages AI models on your machine. [Ollama](/blog/ollama-desktop-app) handles downloading, loading, and running models. InnerZero bundles Ollama and manages it automatically.
...
InnerZero also supports optional cloud AI if you want stronger reasoning. You can bring [your own API keys](/blog/claude-opus-4-7-byo-keys) for providers like OpenAI, Anthropic, Google, or [DeepSeek](/models). Your keys go directly to the provider. InnerZero never touches the API traffic.
