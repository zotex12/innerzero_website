...
The sandbox is enforced at the file-access layer in InnerZero, independent of which model provides the reasoning. The same sandbox rules apply with a local Qwen3 model or cloud Claude Opus 4.8 via BYO. The model changes; the safety model doesn't.
