...
The sandbox is enforced at the file-access layer in InnerZero, independent of which model provides the reasoning. The same sandbox rules apply with local qwen2.5-coder or cloud Claude Opus 4.7 via BYO. The model changes; the safety model doesn't.
