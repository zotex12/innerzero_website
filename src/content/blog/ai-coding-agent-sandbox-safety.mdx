...
**InnerZero's [coding agent](/features/specialists) is restricted to explicit file paths you grant for the task, can't execute code, and requires approval before any write touches disk.** The sandbox is enforced at the path-validation layer, so the agent physically cannot access files outside what you've granted.
...
The sandbox is enforced at the file-access layer in InnerZero, independent of which model provides the reasoning. The same sandbox rules apply with a local Qwen3 model or cloud Claude Opus 4.7 via BYO. The model changes; the safety model doesn't.
