...
**Cursor routes through frontier cloud models (Claude, GPT, Gemini) by default. InnerZero runs local open-source models by default and adds optional cloud via BYO keys.** Both can reach the latest frontier Claude and GPT models. The path to get there is different.
...
- Anyone running on hardware good enough to host a serious local model (16GB+ RAM, 8GB+ VRAM)
...
| Free tier | Full app, free to download | Limited free, then Pro |
