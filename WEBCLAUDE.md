...
| **Fix** | Mobile nav transparent header — Header bg was `bg-transparent` when at page top, making hamburger menu unreadable after navigating. Lifted MobileNav `open` state to Header component; header now gets solid `bg-bg-primary/80 backdrop-blur-md` background when mobile menu is open OR when scrolled. MobileNav refactored to accept `open`/`onOpenChange` props. Mobile audit confirmed: Container px-4 padding, responsive grids, overflow-hidden on Hero glow, footer 2-col grid, terms max-w-3xl all correct | COMPLETE 2026-04-06 |
