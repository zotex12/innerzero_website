...
| **Fix** | Mobile nav transparent menu — previous fix only made the header bar solid, but the actual menu panel was a narrow `w-72` slide-in with semi-transparent `bg-black/50` backdrop, letting hero text bleed through. Replaced with full-screen opaque overlay (`fixed inset-0 bg-bg-primary`), removed separate backdrop div, added `overflow-y-auto` for scrollability, increased tap targets (`py-3` + `hover:bg-bg-secondary`), header row matches `h-16`. Slides in from right via existing `translateX` animation. Solid on both dark and light themes. Header `bg-bg-primary/80` on open from previous fix still active | COMPLETE 2026-04-06 |
